...
public class CDMStructureValidator {
    ...
    public void validateFile(String filePath) {
        ...
        try {
            ...
            System.out.println("[OK] Detected file type: " + detectedType);
            ...
            System.out.println("\n" + "=".repeat(60));
            System.out.println("[OK] Validation completed. See diagnostics above.");
            
        } catch (Exception e) {
            System.err.println("[FATAL] Fatal error during validation: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private JsonNode loadAndParseJson(String filePath) {
        try {
            ...
            System.out.println("[OK] JSON is well-formed");
            ...
            System.err.println("[FAIL] Failed to load or parse JSON file: " + e.getMessage());
            return null;
        }
    }
    ...
    private <T extends RosettaModelObject> boolean tryLoadAs(Class<T> clazz, String filePath, String typeName) {
        try {
            ...
            System.out.println("[OK] Successfully loaded as " + typeName);
            
            try {
                T resolved = ResourcesUtils.getObjectAndResolveReferences(clazz, filePath);
                System.out.println("[OK] Reference resolution successful for " + typeName);
                validateSpecificType(resolved, typeName);
                return true;
                
            } catch (Exception e) {
                System.out.println("[WARN] Reference resolution failed for " + typeName + ": " + e.getMessage());
                validateSpecificType(object, typeName);
                return true;
            }
            
        } catch (Exception e) {
            System.out.println("[FAIL] Failed to load as " + typeName + ": " + e.getMessage());
            return false;
        }
    }
    ...
    private void validateTradeState(TradeState tradeState) {
        if (tradeState.getTrade() == null) {
            ...
            System.out.println("    [OK] Trade object exists");
            validateTrade(tradeState.getTrade());
        }
        
        if (tradeState.getState() == null) {
            System.out.println("    [WARN] State is null (may be optional depending on use case)");
        } else {
            System.out.println("    [OK] State exists: " + tradeState.getState().getPositionState());
        }
    }
    ...
    private void validateTrade(Trade trade) {
        ...
        if (trade.getTradeIdentifier() == null || trade.getTradeIdentifier().isEmpty()) {
            ...
            System.out.println("      [OK] Trade identifiers found: " + trade.getTradeIdentifier().size());
            validateTradeIdentifiers(trade.getTradeIdentifier());
        }
        
        if (trade.getTradeDate() == null) {
            System.out.println("      [WARN] Trade date is null");
        } else {
            System.out.println("      [OK] Trade date: " + trade.getTradeDate().getValue());
        }
        
        if (trade.getTradableProduct() == null) {
            System.out.println("      [CRITICAL] Tradable product is null");
        } else {
            System.out.println("      [OK] Tradable product exists");
        ...
        if (trade.getParty() == null || trade.getParty().isEmpty()) {
            ...
            System.out.println("      [OK] Parties found: " + trade.getParty().size());
            validateParties(trade.getParty());
        }
        
        // Detailed counterparty analysis for DRR compatibility
        validateCounterpartyStructure(trade);
    }
    ...
    private void validateParties(List<? extends Party> parties) {
        for (int i = 0; i < parties.size(); i++) {
            ...
            if (party.getPartyId() == null || party.getPartyId().isEmpty()) {
                System.out.println("          [WARN] No party IDs");
            } else {
                System.out.println("          [OK] Party IDs: " + party.getPartyId().size());
            }
            
            if (party.getName() == null) {
                System.out.println("          [WARN] No party name");
            } else {
                System.out.println("          [OK] Party name: " + party.getName().getValue());
            }
        }
    }

    private void validateCounterpartyStructure(Trade trade) {
        System.out.println("    -> Detailed Counterparty Analysis:");
        
        // Check TradableProduct counterparties
        if (trade.getTradableProduct() != null && trade.getTradableProduct().getCounterparty() != null) {
            System.out.println("      [OK] TradableProduct has counterparties: " + trade.getTradableProduct().getCounterparty().size());
            
            for (int i = 0; i < trade.getTradableProduct().getCounterparty().size(); i++) {
                var counterparty = trade.getTradableProduct().getCounterparty().get(i);
                System.out.println("        Counterparty " + (i + 1) + ":");
                System.out.println("          Role: " + (counterparty.getRole() != null ? counterparty.getRole() : "NULL"));
                System.out.println("          Party Reference: " + (counterparty.getPartyReference() != null ? "EXISTS" : "NULL"));
                
                if (counterparty.getPartyReference() != null) {
                    var partyRef = counterparty.getPartyReference().getValue();
                    if (partyRef != null) {
                        System.out.println("          Party Name: " + (partyRef.getName() != null ? partyRef.getName().getValue() : "NULL"));
                        System.out.println("          Party IDs: " + (partyRef.getPartyId() != null ? partyRef.getPartyId().size() : 0));
                    } else {
                        System.out.println("          Party Reference Value: NULL");
                    }
                }
            }
        } else {
            System.out.println("      [CRITICAL] TradableProduct has NO counterparties!");
            System.out.println("      -> This is likely the cause of ExtractTradeCounterparty returning null");
        }
        
        // Check root-level parties
        if (trade.getParty() != null && !trade.getParty().isEmpty()) {
            System.out.println("      [INFO] Root-level parties: " + trade.getParty().size());
            for (int i = 0; i < trade.getParty().size(); i++) {
                var party = trade.getParty().get(i);
                System.out.println("        Root Party " + (i + 1) + ": " + 
                    (party.getName() != null ? party.getName().getValue() : "Unnamed"));
            }
        } else {
            System.out.println("      [WARN] No root-level parties found");
        }
        
        // Provide specific guidance
        System.out.println("      -> DRR Requirements:");
        System.out.println("         • TradableProduct must have counterparties with proper roles");
        System.out.println("         • Each counterparty must have a valid partyReference");
        System.out.println("         • Common roles: PARTY_1, PARTY_2, BUYER, SELLER");
    }
    ...
    private void validateForDRR(String filePath, String detectedType) {
        ...
        if (trade == null) {
            ...
            System.out.println("[OK] Trade object can be extracted for DRR");
    ...
    private void validateDRRRequirements(Trade trade) {
        ...
        // Check trade identifiers safely
        if (trade.getTradeIdentifier() == null) {
            System.out.println("    [CRITICAL] Trade identifiers are null - this will cause null pointer exceptions");
            System.out.println("    -> DRR requires valid trade identifiers for processing");
        } else if (trade.getTradeIdentifier().isEmpty()) {
            System.out.println("    [CRITICAL] Trade identifiers are empty - DRR requires at least one identifier");
        } else {
            ...
            if (!hasUTI) {
                System.out.println("    [WARN] No UTI (Unique Transaction Identifier) found");
                System.out.println("    -> UTI is recommended for regulatory reporting");
            } else {
                System.out.println("    [OK] UTI found");
            }
            System.out.println("    [OK] Trade identifiers exist: " + trade.getTradeIdentifier().size());
        }
        
        if (trade.getTradeDate() == null) {
            System.out.println("    [WARN] Trade date missing - may affect event timestamp generation");
        } else {
            System.out.println("    [OK] Trade date exists: " + trade.getTradeDate().getValue());
        }
        
        if (trade.getParty() == null) {
            System.out.println("    [CRITICAL] Parties are null - DRR requires parties for reporting");
        } else if (trade.getParty().size() < 2) {
            System.out.println("    [WARN] Less than 2 parties (" + trade.getParty().size() + ") - may affect reporting party identification");
        } else {
            System.out.println("    [OK] Parties found: " + trade.getParty().size());
        }
        
        // Check tradable product
        if (trade.getTradableProduct() == null) {
            System.out.println("    [CRITICAL] TradableProduct is null - required for DRR processing");
        } else {
            System.out.println("    [OK] TradableProduct exists");
        }
        
        System.out.println("    [INFO] DRR requirements check completed");
    }
